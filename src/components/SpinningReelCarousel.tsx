...
export function SpinningReelCarousel({ 
  ...
  // Calculate positions with better accuracy - fix center alignment
  const winningPosition = getWinningPosition()
  const centerPosition = itemWidth * Math.floor(visibleItems / 2) // Center position for container
  ...
  const targetTransform = -(validWinningPosition * itemWidth) + centerPosition // Perfect center alignment
  ...
  return (
    <div className="relative w-full h-64 overflow-hidden bg-gradient-to-r from-orange-900/50 via-orange-800/50 to-orange-900/50 rounded-2xl border-4 border-orange-400/60 shadow-2xl mx-auto max-w-5xl">
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <motion.div
          ...
          style={{ 
            width: `${itemWidth * Math.max(visibleItems, reelItems.current.length)}px`
          }}
        initial={{ x: 0 }}
        animate={{
        ...
        {reelItems.current.map((item, index) => {
          ...
          return (
            <motion.div
              ...
              className={`flex-shrink-0 mx-2 rounded-2xl flex flex-col items-center justify-center transition-all duration-300 backdrop-blur-md ${
                isInCelebration 
            ...
            >
              <div className="flex flex-col items-center justify-center h-full p-2">
                <motion.div 
                  className={`w-28 h-28 mb-1 flex items-center justify-center transition-all duration-300 ${
